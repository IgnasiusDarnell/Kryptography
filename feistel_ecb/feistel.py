...
def encryptMessage(key, message, mode):
    ciphertext = ""
    n = BLOCKSIZE  # 8 bytes (32 bits) per block

    # Split mesage into 32bit blocks
...
def decryptCipher(key, ciphertext, mode):
    ...
    # Split message into 32-bit blocks
    ciphertext = [ciphertext[i: i + n] for i in range(0, len(ciphertext), n)]

    # Adjusting block size comment for consistency
    ...
    for block in ciphertext:
        ...
        for i in range(ROUNDS, 0, -1):  # Correcting loop range
        ...
        # Append decrypted block to message
        message += (L[0] + R[0])

    return message
def key_32(key):
    key = key.encode('utf-8')  # Mengonversi string ke byte sebelum hashing
...
def scramble(x, i, k):
    x = int.from_bytes(x.encode(), 'big')  # Konversi string huruf ke bilangan bulat menggunakan byte encoding
    k = int.from_bytes(k.encode(), 'big')  # Konversi string huruf ke bilangan bulat menggunakan byte encoding

    res = (x * k) % (2**16)
    res = format(res, '04x')  # Format hasil menjadi string heksadesimal dengan panjang 4 karakter
    return res
